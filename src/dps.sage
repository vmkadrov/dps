...
class Scheme:
    def __init__(
        self, left: list, ops: list, param, sch, order, n_val=var("n_val"), delta=None
    ):
        self.scheme = sch
        self.dvar = left[0]
        self.ivar = left[1]
    ...
    def subs(self, tree: Tree, func, f, x, coef):
        ...
        if tree.left == None and tree.right == None:
            ...
            return node(
                self.subs(tree.left, func, f, x, coef),
                self.subs(tree.right, func, f, x, coef),
            )
    ...
    def __add_to_graph(self, tree: Tree, func, f, x, coef, g: Graph, cnt):
        ...
        if tree.right:
            g.add_path(
                [
                    f"{init}, {self.__subs(tree, func, f, x, coef)}",
                    f"{cnt[0] + 1}, {self.__subs(tree.right, func, f, x, coef)}",
                ]
            )
            cnt[0] = cnt[0] + 1
        ...
        if tree.left:
            g.add_path(
                [
                    f"{init}, {self.__subs(tree, func, f, x, coef)}",
                    f"{cnt[0] + 1}, {self.__subs(tree.left, func, f, x, coef)}",
                ]
            )
            cnt[0] = cnt[0] + 1
    ...
    def __diff_coef(self, k, G, SS, c):
        if k == 0:
            ...
            solved = solve(
                diff(G, self.delta, k).subs([self.delta == 0]).subs(SS_pred), c[k]
            )
            SS = SS_pred + solved
        return SS
    ...
    def __create_tree(self, expr):
        ...
        if not op:
            ...
            if len(operands) > 2:
                return Tree(
                    op,
                    left=self.__create_tree(operands[0]),
    ...
    def parametric_eqs(self, right=None):
        ...
        if self.op:
            scheme = scheme.substitute_function(self.op, D)
        ...
        S = [
            self.n_val
            == sum(
                [cc * tt for (cc, tt) in zip(c, [self.delta ^ n for n in range(k + 1)])]
            )
        ]
    ...
    def coefficients(self, right):
        ...
        for i in range(len(Y)):
            ...
            for j in range(len(list(K(Y[i].operands()[1])))):
                org = (
                    K(Y[i].operands()[0]).list()[j]
                    if j < len(K(Y[i].operands()[0]).list())
                    else 0
                )
...
def dps(problem, scheme: Scheme, coef: list, N=10):
    ...
    for n in range(N):
        t0 = t0 + dt
        x0 = [
            xx + problem.subs(sch[ff] * dt, x0).subs([scheme.delta == dt])
            for (xx, ff) in zip(x0, f)
        ]
        l.append([t0] + x0)
